...
// ==== Challenge 2: Use .map() ====
// The event director needlet fullName = [];
let allCaps = [];
...
// Problem 1
// get the emails and names of everyone running

nameAndemail = [];
runners.filter(runner => nameAndemail.push(`${runner.first_name} ${runner.last_name} ${runner.email}`));
console.log(nameAndemail);
// Problem 2
// Get the companyies who donated and the amount given for the race
company = [];
runners.filter(runner => company.push(`${runner.company_name} ${runner.donation}`));
console.log(company);
// Problem 3